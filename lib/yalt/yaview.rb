...
module YALTools
  ...
  class YaDocs
    ...
    #     +------------+-------+------------------+----------+
    #
    def yield_skip_page(limit, total_rows, start_page=1) # :yields: limit, skip, page, max_page
      ...
      while page <= max_page
        if page == max_page
          tmp_r = (total_rows % limit)
          yield [tmp_r == 0 ? limit : tmp_r, skip,page,max_page]
        else
          yield [limit,skip,page,max_page]
        end
        skip = limit * page
        page += 1
      end
